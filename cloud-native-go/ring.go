...
// Push appends an item in the ring buffer and drains (copies items and sends to Consumer) if full.
...
// ringBuffer stores multiple buffers (stripes) and distributes Pushed items between them to lower contention.
type ringBuffer struct {
	pool *sync.Pool
}

// newRingBuffer returns a striped ring buffer.
// The Consumer in ringConfig will be called when individual stripes are full and need to drain their elements.
func newRingBuffer(cons ringConsumer, capa int64) *ringBuffer {
	...
	// We do lose some stripes due to GC (unheld items in sync.Pool are cleared), but the performance gains generally outweigh the small percentage of elements lost.
	// The performance primarily comes from low-level runtime functions used in the standard library that aren't available to us (such as runtime_procPin()).
	return &ringBuffer{
		pool: &sync.Pool{
...
// Push adds an element to one of the internal stripes and possibly drains if the stripe becomes full.
